...
function generateLLMS(): string {
  ...
  `## Product`,
    ...
    ``,
    `## Blog (how-to guides & comparisons)`,
    ``,
    ...blogPosts.map(
      (p) => `- [${p.metaTitle ?? p.title}](${SITE_URL}/blog/${p.slug}): ${p.description}`
    ),
    ``,
    `## Industry Use Cases`,
    ``,
    ...industries.map((ind) => `- [${ind.metaTitle}](${SITE_URL}/industries/${ind.slug})`),
    ``,
    `## Help & Contact`,
    ``,
    ...
    `- [ai.txt](${SITE_URL}/ai.txt): Key platform facts in plain key-value lines for quick machine parsing.`,
